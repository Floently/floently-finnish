 ...
 *   - Uses inline CSS variables so the same component can theme into dark
 *     (landing hero) or light (auth) contexts.
 *   - Self-cleaning: clears all timers on unmount.
 *
...
const TIMINGS: Record<Phase, number> = {
  ...
  pause: 600,
  error_flag: 800,
  tooltip: 2600,    // slightly longer so users can read the rule
  correcting: 700,
  success: 1600,
  reset: 600,
};
...
export default function FinnishCorrectionDemo({ theme = 'dark', compact = false }: Props) {
  ...
  const { copy } = usePublicPageI18n();
  const demo = copy.demo;
  const sentence = demo.sentence;
  const wrongWord = demo.wrongWord;
  const rightWord = demo.rightWord;
  ...
  useEffect(() => {
    ...
    const advance = () => {
      ...
      } else if (current === 'reset' || current === 'cursor') {
        setTypedChars(0);
      } else {
        setTypedChars(sentence.length);
      }
  ...
  const themeVars: React.CSSProperties = theme === 'dark'
    ? {
        ['--demo-bg' as string]: 'rgba(17, 35, 70, 0.55)',
        ['--demo-border' as string]: 'rgba(90, 133, 255, 0.20)',
  ...
  return (
    <div className="floently-demo-root" style={themeVars}>
      <DemoStyles />
      ...
      <div className={`floently-demo-input ${showCheck ? 'is-success' : ''}`}>
        <div className="floently-demo-prompt">{demo.prompt}</div>
        <div className="floently-demo-text">
          {phase === 'cursor' || phase === 'reset' ? (
        ...
        <div
          ...
          style={{ opacity: showTooltip ? 1 : 0, transform: showTooltip ? 'translateX(-50%) translateY(0)' : 'translateX(-50%) translateY(6px)' }}
          aria-live="polite"
        >
          <div className="floently-demo-tooltip-arrow" />
          <div className="floently-demo-tooltip-title">{demo.tooltipTitle}</div>
  ...
  return (
    <>
      ...
      <span className={`floently-demo-target ${flaggedAsError ? 'is-error' : ''}`}>
        <span
          className="floently-demo-target-wrong"
          style={{
        ...
        <span
          className="floently-demo-target-right"
          style={{
  ...
  return (
    <style>{`
      ...
      .floently-demo-label {
        ...
        text-transform: uppercase; color: var(--demo-label);
        margin-bottom: 14px;
      }
      .floently-demo-dot {
      ...
      .floently-demo-prompt {
        ...
        text-transform: uppercase; color: var(--demo-text-muted);
        margin-bottom: 12px;
      }
      .floently-demo-text {
        font-size: 24px; font-weight: 600; letter-spacing: -0.2px;
        line-height: 1.45; color: var(--demo-text); min-height: 36px;
      }
      ...
      .floently-demo-target-right { color: var(--demo-success); font-weight: 700; }
      ...
      .floently-demo-tooltip-body {
        font-size: 12px; color: var(--demo-text-muted); line-height: 1.55;
      }
      ...
      .floently-demo-check {
        ...
        background: var(--demo-success); color: #ffffff;
        font-size: 12px; font-weight: 800;
      }

      @media (max-width: 767px) {
        .floently-demo-tooltip { max-width: 280px; font-size: 12px; }
        .floently-demo-text { font-size: 20px; }
      }
