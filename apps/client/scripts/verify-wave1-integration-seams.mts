...
function composeOne(task: any, profession = 'nurse') {
  return composePracticeSession({ learnerId: 'integration-learner', createdAt: '2026-08-17T17:00:00.000Z', scope: task.pathway, targetMinutes: 20, candidates: [task], entitlements: entitlementsFor(task), profession, modalities: allModalities, enabledFeatureFlags: task.featureFlag ? [task.featureFlag] : [], evidence: [] });
}
...
for (const descriptor of missionDescriptors) {
  ...
  const mismatch = composePracticeSession({ learnerId: 'integration-learner', createdAt: '2026-08-17T17:00:00.000Z', scope: 'professional', targetMinutes: 20, candidates: [descriptor], entitlements: ['professionalAccess', `profession:${descriptor.profession}`], profession: wrongProfession, modalities: allModalities, enabledFeatureFlags: descriptor.featureFlag ? [descriptor.featureFlag] : [], evidence: [] });
