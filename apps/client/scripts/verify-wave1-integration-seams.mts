...
for (const descriptor of readingDescriptors) {
  ...
  assert.deepEqual(result.manifest.tasks.map((item) => item.task.taskId), [descriptor.taskId], `Practice must accept integrated Reading descriptor ${descriptor.taskId}`);
}
...
assert.deepEqual(composeOne(professionalReading, 'doctor').manifest.tasks.map((item) => item.task.taskId), [professionalReading.taskId], 'Generic Professional Reading must remain schedulable in an entitled profession context');
...
for (const descriptor of writingDescriptors) {
  ...
  assert.deepEqual(result.manifest.tasks.map((item) => item.task.taskId), [descriptor.taskId], `Practice must accept integrated Writing descriptor ${descriptor.taskId}`);
}
...
for (const descriptor of missionDescriptors) {
  ...
  assert.equal(result.manifest.tasks.length, 0, `Unresolved/degraded mission descriptor ${descriptor.taskId} must fail closed in ordinary Practice`);
  assert.ok(result.diagnostics.some((diagnostic) => diagnostic.taskId === descriptor.taskId && ['unavailable', 'degraded_excluded'].includes(diagnostic.code)), `Mission descriptor ${descriptor.taskId} must expose truthful health diagnostics`);
  ...
  assert.ok(mismatch.diagnostics.some((diagnostic) => diagnostic.taskId === descriptor.taskId && diagnostic.code === 'profession_mismatch'), `Mission descriptor ${descriptor.taskId} must remain profession-isolated`);
}
